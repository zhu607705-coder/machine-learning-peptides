...
第二类数据来源是围绕 `final purity / isolated yield` 目标的文献人工抽表数据集。项目通过联网检索和逐篇整理，建立了 `data/real/final_purity_yield_literature.csv`，并以 `source_id` 标记研究来源。该数据集当前共整理 `220` 条来源可追踪记录，覆盖 `18` 个 primary sources，其中新增了 `parallel_pec_2021` 这类以 `purified_hplc + recovery` 为主的纯化文献条目，也补齐了 `Human Beta Defensin 3` 的 canonical sequence 与 `isolated` basis 映射。此外，还补充了 `Accelerated Multiphosphorylated Peptide Synthesis (AMPS)` (2022) 的 8 条多磷酸化肽记录和 `Universal Peptide Synthesis via Solid-Phase Methods fused with Automation` (Nature Communications 2025) 的 7 条自动化平台记录。若按当前 target-specific ingestion 进入建模流程，则共有 `203` 条样本至少包含一个有效目标值。<sup>[8-15,20-21]</sup>
...
[20] Accelerated Multiphosphorylated Peptide Synthesis (AMPS): a platform for rapid synthesis of multiply phosphorylated peptides[J/OL]. ACS Chemical Science, 2022[2026-03-03]. Available: https://pmc.ncbi.nlm.nih.gov/articles/PMC9397535/.  
[21] Universal peptide synthesis via solid-phase methods fused with automation[J/OL]. Nature Communications, 2025[2026-03-03]. Available: https://www.nature.com/articles/s41467-025-62344-2.  
